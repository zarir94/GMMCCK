...
{#snippet ResultBox(r: NonNullable<typeof s.half_yearly_result | typeof s.year_final_result>, type: 'hyr' | 'yfr')}
  <div class="card bg-base-200 border border-base-300 shadow-md mt-5" id="{type}">
    <div class="card-body">
      <div class="flex items-center justify-start gap-3">
        {#if type == 'hyr'}
          <div class="p-2 bg-linear-to-bl from-teal-700 to-blue-700 rounded-md text-white/90"><AwardIcon/></div>
        {:else}
          <div class="p-2 bg-linear-to-bl from-fuchsia-700 to-cyan-700 rounded-md text-white/90"><GemIcon/></div>
        {/if}
        <div class="flex flex-col gap-0.5">
          <h2 class="card-title">{type == 'hyr' ? 'অর্ধ-বার্ষিক': 'বার্ষিক'} পরীক্ষার ফলাফল</h2>
          <h3 class="tracking-wide">{type == 'hyr' ? 'Half-Yearly': 'Year Final'} Exam Result</h3>
          <span class="badge {Number(r.gpa) ? 'badge-info' : 'badge-warning'} badge-xs capitalize font-mono">{Number(r.gpa) ? `GPA ${r.gpa}` : r.gpa}</span>
        </div>
      </div>
          ...
          <li class="group flex items-center justify-between py-3 px-4 bg-base-100 rounded-xl border border-slate-500/20 hover:border-primary/40 hover:bg-primary/5 transition-colors duration-300">
            <div class="flex items-center gap-3">
              ...
              <div>
                ...
                <div class="capitalize text-xs opacity-60 mt-0.75">{d.marks.replaceAll(',', ', ')}</div>
              </div>
            </div>
...
{#if s.half_yearly_result}
  {@render ResultBox(s.half_yearly_result, 'hyr')}
{/if}
{#if s.year_final_result}
  {@render ResultBox(s.year_final_result, 'yfr')}
{/if}
